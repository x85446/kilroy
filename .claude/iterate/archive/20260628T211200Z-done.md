...
- 2026-06-28T21:11Z step7 DONE + ALL COMPLETE. Updated gate --help (STRATEGY ladder
  + 3 modes + family rule) and docs/kilroy-cliproxy-architecture.md (Routing
  strategies section: family table, rungs, default ladder, /v1/responses, .orig
  snapshot, claude-p gated-skip). Added an idle guard to _gate_run_loop so a
  graph-family gate with no active run idles without churning/resuming. Fixed a
  crash: GATE_STRATEGY was referenced in the started-log before _load_gate_config
  (set -u unbound) — added _load_gate_config first. Restarted gate daemon (pid
  12578): log shows "started; strategy=role-split fill-first" then "unit inactive
  on its own (rung=role-split, graph); idling, not auto-resuming" — correct
  graph-family idle, both auths enabled, gate --status running. Deployed md5
  5d62ab79. FINAL SWEEP: all 7 validations GREEN.
