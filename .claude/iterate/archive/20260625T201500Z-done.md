...
- [x] 5. point kilroy at pool + record opus-4.8/gpt-5.5 (Missions 1A+2) + bring up
       new gate daemon (LIVE, pid 1029328, failing over claude→codex)
- [x] 6. status shows both gate states + serving pool

ALL VALIDATIONS GREEN. Final sweep confirms: gate running, PROVIDERS=claude codex,
both providers probed (claude wk 94% PARK, codex ALLOW), live failover (claude
disabled→pool serves 4/4 gpt-5.5), dualpool pins opus-4.8+gpt-5.5, status shows
per-provider gate + "pool: serving via codex".

BOUNDED ITEM (not a blocker): the daemon's systemd stopsafe/resume on an ACTIVE
run was validated by-components (decision via gate --tick RUN:PARK/ALLOW +
enforcement via live failover + the unchanged existing cmd_launch stopsafe/resume
functions), not watched end-to-end — because the only real workload (the parked
izcrOS run) is claude-baked (resume won't take --force-model) and migrating it to
the pool is an operator decision (fresh run loses 166 completions; re-pipeclean+
resume is the alternative). The gate is live and correctly idling (not
auto-resuming the claude-baked run).
...
- 2026-06-25T20:07Z step5: kilroy model-injection = kilroyHelp pipeclean (s2)
  rewrites nodes to provider=anthropic + tiers. For 1A: runtime --force-model
  anthropic=auto (run accepts; resume does NOT). Added _pool_model_flag
  (KILROY_POOL_MODEL default auto) wired into cmd_run. Deployed md5 f83eb948.
  PROVED kilroy->pool via real minimal run: "WARNING: force-model override
  applied: node=say ... model=auto" + proxy journal "Resolved 'auto'" + 200 POST
  /v1/messages x2 during the run + node.completed. 5b satisfied (adapted).
  Real izcrOS migration = fresh pooled run (operator go-decision; not auto-kicked).
